...
**Deadline**: Tuesday, December 2, 2025 at 11:59 PM
...
```bash
git clone ist-git@git.uwaterloo.ca:ece320-f25/project/e3mehta-pd6.git
```
...
Run `make check MEM_PATH=/path/to/program.x` in `build/scripts`.
...
**Step 5.**
On the lab machine, you can clone your repository with `git clone`.
Run `make check_debug MEM_PATH=/path/to/program.x` in `build/scripts`.
...
Ensure that the output trace is correct for all [rv32-benchmarks](https://git.uwaterloo.ca/ece320-f25/rv32-benchmarks).
